...
//    @PostMapping("/login")
    public String loginV3(@Valid @ModelAttribute LoginForm loginForm, BindingResult bindingResult, HttpServletRequest request) {
        ...
        HttpSession session = request.getSession(); //모든과정이 다 된다 ㅎ
        //세션에 로그인 회원정보 보관
        session.setAttribute(SessionConst.LOGIN_MEMBER, loginMember); //세선에 보관하고 싶은 객체를 담아두자.
...
//        expireCookie(response, "memberId");
    ...
    public String logOutV3(HttpServletRequest request) {
        HttpSession session = request.getSession(false); //true면 무조건 만들기 때문에 false로해줘야함.
        if (session != null) {
            session.invalidate(); //세션이랑 여기안에 들어있는 데이터 다 날라감!
        }
        return "redirect:/";
    }
